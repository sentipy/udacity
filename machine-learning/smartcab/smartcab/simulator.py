...
class Simulator(object):
    ...
    def render_text(self, trial, testing=False):
        ...
        # Starting new trial
            ...
            if a.learning:
                print("epsilon = {:.100f}; alpha = {:.4f}".format(a.epsilon, a.alpha))
            else:
        ...
        if status:
            ...
            if (state['destination'] != state['location'] and state['deadline'] > 0) or (self.env.enforce_deadline is not True and state['destination'] != state['location']):
                ...
                if self.env.primary_agent.learning:
                    ...
                    self.screen.blit(self.font.render("epsilon = {:.100f}".format(self.env.primary_agent.epsilon), True, self.colors['black'], self.bg_color), (10, 80))
